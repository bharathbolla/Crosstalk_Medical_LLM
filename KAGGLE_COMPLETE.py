        ...
        "cells": [
            ...
            },
            {
                ...
                "source": [
                    ...
                    "\n",
                    "# ============================================\n",
                    "# Git auth for pushing results back to GitHub\n",
                    "# ============================================\n",
                    "# Setup: Kaggle → Add-ons → Secrets → Add 'GITHUB_TOKEN'\n",
                    "# Value: Your GitHub Personal Access Token (repo scope)\n",
                    "# Create at: GitHub → Settings → Developer Settings → Personal Access Tokens\n",
                    "GITHUB_USER = 'bharathbolla'\n",
                    "GITHUB_REPO = 'Crosstalk_Medical_LLM'\n",
                    "try:\n",
                    ...
                    "    print('⚠️  No GITHUB_TOKEN in Kaggle Secrets — results will NOT be pushed to GitHub')\n",
                    "    print('   To enable: Kaggle → Add-ons → Secrets → Add GITHUB_TOKEN')\n",
                    "\n",
                    "# Clone repo\n",
                    "print('\\n📥 Cloning repository...')\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "\n",
                    "# Save results ALSO to /kaggle/working/ root so they appear in Kaggle Output tab\n",
                    "KAGGLE_OUTPUT_DIR = Path('/kaggle/working')\n",
                    "print(f'📂 Results will be saved to: {RESULTS_DIR} (git) + {KAGGLE_OUTPUT_DIR} (download)')"
                ]
            },
            ...
            },
            {
                ...
                "source": [
                    ...
                    "    # Options: bc2gm, jnlpba, chemprot, ddi, gad, hoc, pubmedqa\n",
                    "    \n",
            ...
            # ==================================================
            {
                ...
                "metadata": {},
                "source": [
                    "## Cell 4: 🔥 SMOKE TEST & MULTI-DATASET VALIDATION\n",
            ...
            },
            {
                ...
                "source": [
                    "# ============================================\n",
                    "# ⭐ SMOKE TEST & MULTI-DATASET TOGGLE\n",
                    "# ============================================\n",
                    "\n",
                    "SMOKE_TEST = True  # ← Change to False for full training\n",
                    "TEST_ALL_DATASETS = False  # ← Change to True to test all 7 datasets\n",
                    "FULL_BATCH_1 = False  # ← Session 1: pubmedqa, ddi, gad, chemprot (~5 hrs)\n",
                    "FULL_BATCH_2 = False  # ← Session 2: bc2gm, hoc, jnlpba (~8 hrs)\n",
                    "\n",
                    "print('\\n' + '='*60)\n",
                    "if FULL_BATCH_1:\n",
                    "    print('🚀 FULL TRAINING - BATCH 1 (4 smaller datasets)')\n",
                    "    print('='*60)\n",
                    "    \n",
                    "    all_datasets = [\n",
                    "        'pubmedqa',    # QA       -  1,000 samples (~30 min)\n",
                    "        'ddi',         # RE       -  4,021 samples (~1 hr)\n",
                    "        'gad',         # Classif  -  4,796 samples (~1.5 hrs)\n",
                    "        'chemprot',    # RE       -  6,437 samples (~2 hrs)\n",
                    "    ]\n",
                    "    \n",
                    ...
                    "    \n",
                    "    print(f'\\nDatasets to train: {len(all_datasets)}')\n",
                    "    for i, ds in enumerate(all_datasets, 1):\n",
                    "        print(f'  {i}. {ds}')\n",
                    "    print('\\nExpected time: ~5 hours on T4')\n",
                    "\n",
                    "elif FULL_BATCH_2:\n",
                    "    print('🚀 FULL TRAINING - BATCH 2 (3 larger datasets)')\n",
                    "    print('='*60)\n",
                    "    \n",
                    "    all_datasets = [\n",
                    "        'bc2gm',       # NER      - 12,574 samples (~3 hrs)\n",
                    "        'hoc',         # Multi-l  - 12,119 samples (~3 hrs)\n",
                    "        'jnlpba',      # NER      - 18,607 samples (~4 hrs)\n",
                    "    ]\n",
                    "    \n",
                    "    CONFIG['all_datasets_to_test'] = all_datasets\n",
                    "    CONFIG['dataset_test_results'] = []\n",
                    ...
                    "    for i, ds in enumerate(all_datasets, 1):\n",
                    "        print(f'  {i}. {ds}')\n",
                    "    print('\\nExpected time: ~8 hours on T4')\n",
                    "\n",
                    "elif TEST_ALL_DATASETS:\n",
                    "    print('🔥 MULTI-DATASET VALIDATION MODE')\n",
                    "    print('='*60)\n",
                    "    \n",
                    "    # All 7 datasets\n",
                    "    all_datasets = [\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "    # Do a proper 90/10 split to avoid data leakage\n",
                    "    print('   ⚠️  No validation split found, splitting train 90/10')\n",
                    "    all_train = raw_data['train'][:]\n",
                    "    random.shuffle(all_train)\n",
                    "    split_idx = int(len(all_train) * 0.9)\n",
                    "    raw_data['train'] = all_train[:split_idx]\n",
                    "    raw_data[val_split] = all_train[split_idx:]\n",
                    "\n",
                    "# Limit samples if smoke test\n",
                    "if max_samples:\n",
                    "    raw_data['train'] = raw_data['train'][:max_samples]\n",
                    "    full_val = raw_data[val_split]\n",
                    "    num_val = max(20, max_samples // 5)\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "else:\n",
                    "    raise ValueError(f\"Unsupported task_type: {task_config['task_type']}\")\n",
                    "\n",
                    "# Create trainer\n",
                    "callbacks = []\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "    all_results = []\n",
                    "    \n",
                    "    # Free Cell 8 model from GPU (if multi-dataset mode)\n",
                    "    try:\n",
                    "        del model, trainer\n",
                    "        torch.cuda.empty_cache()\n",
                    "        gc.collect()\n",
                    "        print('   Freed Cell 8/9 model from GPU')\n",
                    "    except NameError:\n",
                    "        pass\n",
                    "    \n",
                    "    # Resume: load existing results and skip completed datasets\n",
                    "    import pandas as pd\n",
                    "    completed_datasets = set()\n",
                    "    # Look for ANY incremental CSV (handles session restarts with new EXPERIMENT_ID)\n",
                    "    model_short = CONFIG['model_name'].split('/')[-1]\n",
                    "    incremental_csv = RESULTS_DIR / f'results_incremental_{model_short}.csv'\n",
                    "    if incremental_csv.exists():\n",
                    "        prev = pd.read_csv(incremental_csv)\n",
                    "        completed_datasets = set(prev['dataset'].tolist())\n",
                    "        all_results = prev.to_dict('records')\n",
                    "        print(f'📂 Resuming: {len(completed_datasets)} datasets already done: {completed_datasets}')\n",
                    "    \n",
                    ...
                    "        if dataset_to_test in completed_datasets:\n",
                    ...
                    "            val_split = 'validation' if 'validation' in raw_data else 'test'\n",
                    "            if val_split not in raw_data:\n",
                    "                # Proper 90/10 split to avoid data leakage\n",
                    "                print(f'   ⚠️  No val split, splitting train 90/10')\n",
                    "                all_train = raw_data['train'][:]\n",
                    "                random.shuffle(all_train)\n",
                    "                split_idx = int(len(all_train) * 0.9)\n",
                    "                raw_data['train'] = all_train[:split_idx]\n",
                    ...
                    "                    print(f'   ⚠️  WARNING: Highly imbalanced validation set!')\n",
                    "            \n",
                    ...
                    "            if overlapping > 0:\n",
                    "                print(f'   ⚠️  DATA LEAKAGE: {overlapping}/{len(val_texts)} val samples overlap with train!')\n",
                    "            \n",
                    "            # Check for empty texts\n",
                    "            empty_val = sum(1 for s in raw_data[val_split] if not s.get('text', '').strip())\n",
                    "            if empty_val > 0:\n",
                    "                print(f'   ⚠️  EMPTY TEXT: {empty_val}/{len(raw_data[val_split])} val samples have empty text!')\n",
                    "            \n",
                    ...
                    "                    ignore_mismatched_sizes=True\n",
                    "                )\n",
                    "                print(f'   ✅ TokenClassification head loaded ({num_labels} labels)')\n",
                    "            \n",
                    "            elif model_type == 'sequence_classification':\n",
                    "                # RE, Classification, QA\n",
                    "                config = AutoConfig.from_pretrained(model_name)\n",
                    ...
                    "                    ignore_mismatched_sizes=True\n",
                    "                )\n",
                    "                print(f'   ✅ SequenceClassification head loaded ({num_labels} labels)')\n",
                    "            \n",
                    "            elif model_type == 'regression':\n",
                    "                # Similarity\n",
                    "                model = AutoModelForSequenceClassification.from_pretrained(\n",
                    ...
                    "                print(f'   ✅ Regression head loaded')\n",
                    "            else:\n",
                    "                raise ValueError(f'Unsupported model_type: {model_type} for dataset {primary_dataset}')\n",
                    "            \n",
                    "            if torch.cuda.is_available():\n",
                    "                model = model.cuda()\n",
                    "                print('   ✅ Model on GPU')\n",
                    ...
                    "            print(f'\\n⚙️  Setting up trainer...')\n",
                    "            use_es = CONFIG.get('use_early_stopping', False)\n",
                    "            training_args = TrainingArguments(\n",
                    "                output_dir=f\"./checkpoints/{primary_dataset}_{EXPERIMENT_ID}\",\n",
                    "                num_train_epochs=CONFIG['num_epochs'],\n",
                    "                per_device_train_batch_size=CONFIG['batch_size'],\n",
                    ...
                    "                logging_steps=20,\n",
                    "                eval_strategy='epoch',\n",
                    "                save_strategy='epoch' if use_es else 'no',\n",
                    "                save_total_limit=2 if use_es else None,\n",
                    "                load_best_model_at_end=use_es,\n",
                    ...
                    "                    return compute_regression_metrics(predictions, labels)\n",
                    "            else:\n",
                    "                raise ValueError(f\"Unsupported task_type: {task_config['task_type']} for dataset {primary_dataset}\")\n",
                    "            \n",
                    "            # Callback to print + save epoch results\n",
                    "            epoch_results = []  # Collect per-epoch metrics\n",
                    "            class PrintMetricsCallback(TrainerCallback):\n",
                    "                def on_evaluate(self, args, state, control, metrics=None, **kwargs):\n",
                    "                    if metrics:\n",
                    "                        epoch = int(state.epoch) if state.epoch else 0\n",
                    "                        f1 = metrics.get('eval_f1', metrics.get('eval_exact_match', 0))\n",
                    "                        loss = metrics.get('eval_loss', 0)\n",
                    "                        prec = metrics.get('eval_precision', 0)\n",
                    "                        rec = metrics.get('eval_recall', 0)\n",
                    "                        print(f'  Epoch {epoch}: loss={loss:.4f}, F1={f1:.4f}, P={prec:.4f}, R={rec:.4f}')\n",
                    ...
                    "            \n",
                    "            # Setup callbacks\n",
                    "            cb_list = [PrintMetricsCallback()]\n",
                    "            if use_es:\n",
                    "                cb_list.append(EarlyStoppingCallback(early_stopping_patience=CONFIG.get('early_stopping_patience', 3)))\n",
                    ...
                    "            \n",
                    "            # Save results incrementally (don't lose progress if session dies)\n",
                    "            # Save dataset-level results (keyed by model name for resume across sessions)\n",
                    "            import pandas as pd\n",
                    "            df_incremental = pd.DataFrame(all_results)\n",
                    "            df_incremental.to_csv(incremental_csv, index=False)\n",
                    "            import shutil\n",
                    "            shutil.copy2(incremental_csv, KAGGLE_OUTPUT_DIR / incremental_csv.name)\n",
                    "            \n",
                    "            # Save epoch-level results\n",
                    "            if epoch_results:\n",
                    "                df_epochs = pd.DataFrame(epoch_results)\n",
                    "                epoch_csv = RESULTS_DIR / f'epoch_results_{model_short}.csv'\n",
                    "                # Append to existing file if resuming\n",
                    "                if epoch_csv.exists():\n",
                    "                    existing = pd.read_csv(epoch_csv)\n",
                    "                    df_epochs = pd.concat([existing, df_epochs], ignore_index=True)\n",
                    "                df_epochs.to_csv(epoch_csv, index=False)\n",
                    "                shutil.copy2(epoch_csv, KAGGLE_OUTPUT_DIR / epoch_csv.name)\n",
                    "            \n",
                    "            print(f'💾 Results saved ({len(all_results)} datasets so far)')\n",
                    "            \n",
                    "            # Git commit and push results\n",
                    "            # Files are already in the repo (RESULTS_DIR is inside the cloned repo)\n",
                    "            if GITHUB_TOKEN:\n",
                    "                try:\n",
                    "                    import subprocess\n",
                    "                    repo_dir = Path('/kaggle/working/Crosstalk_Medical_LLM')\n",
                    "                    subprocess.run(['git', 'add', 'results/'], cwd=str(repo_dir), check=True)\n",
                    "                    result = subprocess.run(['git', 'commit', '-m', f'Results: {primary_dataset} complete ({model_short})'], cwd=str(repo_dir), capture_output=True, text=True)\n",
                    "                    if result.returncode == 0:\n",
                    "                        subprocess.run(['git', 'push'], cwd=str(repo_dir), check=True)\n",
                    "                        print(f'📤 Results pushed to GitHub')\n",
                    "                    else:\n",
                    "                        print(f'⚠️  Nothing to commit (no new changes)')\n",
                    "                except Exception as git_err:\n",
                    "                    print(f'⚠️  Git push failed: {git_err} (results still saved locally)')\n",
                    "            else:\n",
                    "                print(f'⚠️  Skipping git push (no GITHUB_TOKEN). Results saved locally.')\n",
                    "            \n",
                    "            # Clean up GPU memory and checkpoints\n",
                    "            import shutil\n",
                    "            ckpt_dir = f\"./checkpoints/{primary_dataset}_{EXPERIMENT_ID}\"\n",
                    ...
                    "            })\n",
                    "            \n",
                    "            # Save error to CSV (so resume skips this dataset)\n",
                    "            try:\n",
                    "                df_err = pd.DataFrame(all_results)\n",
                    "                df_err.to_csv(incremental_csv, index=False)\n",
                    "                shutil.copy2(incremental_csv, KAGGLE_OUTPUT_DIR / incremental_csv.name)\n",
                    "                print(f'💾 Error recorded to CSV')\n",
                    "            except Exception:\n",
                    "                pass\n",
                    "            \n",
                    "            # Clean up GPU memory on error too (prevent OOM on next dataset)\n",
                    "            try:\n",
                    "                del model\n",
                    "            except NameError:\n",
                    "                pass\n",
                    ...
                    "    \n",
                    "    # Save results\n",
                    "    results_file = RESULTS_DIR / f'dataset_validation_{model_short}.csv'\n",
                    "    df_results.to_csv(results_file, index=False)\n",
                    "    import shutil\n",
                    "    shutil.copy2(results_file, KAGGLE_OUTPUT_DIR / results_file.name)\n",
                    "    print(f'\\n💾 Results saved to: {results_file}')\n",
                    "    print(f'📥 Downloadable from: {KAGGLE_OUTPUT_DIR / results_file.name}')\n",
                    "    \n",
                    "    # Final verdict\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "    \n",
                    "    # Copy to /kaggle/working for download from Output tab\n",
                    "    import shutil\n",
                    "    shutil.copy2(results_file, Path('/kaggle/working') / results_file.name)\n",
                    "    \n",
                    "    print(f'\\n✅ Results saved to: {results_file}')\n",
                    "    print(f'📥 Downloadable from: /kaggle/working/{results_file.name}')\n",
                    "else:\n",
                    "    print('\\n⏭️  Skipping - Multi-dataset results already saved in Cell 10')"
                ]
            },
            ...
            },
            {
                ...
                "source": [
                    ...
                    "            print('   → Ready for full experiments (7 models × 8 tasks)')\n",
                    "        else:\n",
                    "            print(f'\\n⚠️  {total - passed} datasets failed')\n",
                    "            print('   → Review results above and fix issues')\n",
                    "    \n",
                    "    print('\\n' + '='*60)\n",
                    "    print('Next Steps:')\n",
